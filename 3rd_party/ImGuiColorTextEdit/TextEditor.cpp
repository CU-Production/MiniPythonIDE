...
// Compatibility macros for ImGui API changes (1.87+)
#if IMGUI_VERSION_NUM >= 18700
    #define ImGui_GetKeyIndex(key) (key)
    #define ImGui_PushAllowKeyboardFocus ImGui::PushTabStop
    #define ImGui_PopAllowKeyboardFocus ImGui::PopTabStop
#else
    #define ImGui_GetKeyIndex ImGui::GetKeyIndex
    #define ImGui_PushAllowKeyboardFocus ImGui::PushAllowKeyboardFocus
    #define ImGui_PopAllowKeyboardFocus ImGui::PopAllowKeyboardFocus
#endif
...
void TextEditor::HandleKeyboardInputs()
{
	...
	if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Z)))
		Undo();
	else if (!IsReadOnly() && !ctrl && !shift && alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Backspace)))
		Undo();
	else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Y)))
		Redo();
	else if (!ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_UpArrow)))
		MoveUp(1, shift);
	else if (!ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_DownArrow)))
		MoveDown(1, shift);
	else if (!alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_LeftArrow)))
		MoveLeft(1, shift, ctrl);
	else if (!alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_RightArrow)))
		MoveRight(1, shift, ctrl);
	else if (!alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_PageUp)))
		MoveUp(GetPageSize() - 4, shift);
	else if (!alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_PageDown)))
		MoveDown(GetPageSize() - 4, shift);
	else if (!alt && ctrl && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Home)))
		MoveTop(shift);
	else if (ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_End)))
		MoveBottom(shift);
	else if (!ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Home)))
		MoveHome(shift);
	else if (!ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_End)))
		MoveEnd(shift);
	else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Delete)))
		Delete();
	else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Backspace)))
		Backspace();
	else if (!ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Insert)))
		mOverwrite ^= true;
	else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Insert)))
		Copy();
	else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_C)))
		Copy();
	else if (!IsReadOnly() && !ctrl && shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Insert)))
		Paste();
	else if (!IsReadOnly() && ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_V)))
		Paste();
	else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_X)))
		Cut();
	else if (!ctrl && shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Delete)))
		Cut();
	else if (ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_A)))
		SelectAll();
	else if (!IsReadOnly() && !ctrl && !shift && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Enter)))
		EnterCharacter('\n', false);
	else if (!IsReadOnly() && !ctrl && !alt && ImGui::IsKeyPressed(ImGui_GetKeyIndex(ImGuiKey_Tab)))
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
	...
	// Automatically get focus when the window is clicked
	...
	{
		ImGui_PushAllowKeyboardFocus(true);
